...
# email, phone, credit card, IBAN, IP) plus our one domain entity.
ENTITIES = [
    "PERSON",
    "EMAIL_ADDRESS",
    "PHONE_NUMBER",
...
def _build_analyzer() -> AnalyzerEngine:
    """Default analyzer plus the domain custom recogniser."""
    ...
    # ============================================================ TASK 2
    # This deployment holds medical record numbers, shaped MRN-4417-2026, and
    # no general-purpose detector has heard of them. Register a recogniser for
    # the MRN entity here.
    #
    # Three decisions the tests check, and that you should be able to defend:
    #
    #   * Word boundaries, NOT anchors. This is a SCANNER, meant to find every
    #     identifier in a sentence, not a validator checking that a whole
    #     string is one. An anchored pattern fails silently: no error, no
    #     detection, and a note that looked scrubbed.
    #   * A base score that clears the redaction threshold on its own. This is
    #     your identifier, with a shape you defined.
    #   * A context list, so the pattern can stay strict without losing recall.
    #
    # `settings.mrn_score` is already in config. Use it rather than a literal,
    # so the number is reviewable in one place.
    #
    #     pytest -q tests/test_safeassist.py -k task2
    # ============================================================
    return analyzer
...
def startup_check() -> None:
    ...
    probe = "Contact John Doe at john@example.com about patient record MRN-1234-5678."
    results = analyzer.analyze(text=probe, language="en", entities=ENTITIES)
    entities_found = {r.entity_type for r in results}
    # TASK 2 also lands here. A startup probe verifies what the deployment
    # CLAIMS to protect, so a new recogniser that is not in this set is a
    # control the boot will happily start without. Add "MRN" when you register
    # it, and watch this crash before you write the recogniser: that crash is
    # the probe doing its job.
    required = {"PERSON", "EMAIL_ADDRESS"}
...
def scrub_input(text: str) -> ScrubResult:
    """Call site 1 of 4: the user's query, before anything else sees it."""
    return _scrub(text, surface="input")


def scrub_output(text: str) -> ScrubResult:
    """Call site 2 of 4: one buffered sentence, before it is streamed out."""
    return _scrub(text, surface="output")


def scrub_chunk(text: str) -> ScrubResult:
    """Call site 3 of 4: INGESTION. Runs in `app/ingest.py`, before the chunk
    is embedded and upserted. Unredacted PII never lands in the store."""
    return _scrub(text, surface="retrieval")


def scrub_telemetry(text: str) -> ScrubResult:
    """Call site 4 of 4: OPERATIONAL LOGGING, which is an egress path too.

    Same detector, same thresholds, a different surface label so the audit
    trail can say which path a redaction happened on. GuardianAI had three call
    sites and left this one open; closing it is why this function exists.
    """
    return _scrub(text, surface="telemetry")
