...
from app import audit, review, telemetry, eval as eval_mod, ingest as ingest_mod, llm, rbac, store
...
from app.schemas import (AskRequest, Citation, EvalReport, IngestTextRequest, InjectionVerdict,
                         RetrievedChunk, ReviewDecision)

logging.basicConfig(
    level=get_settings().log_level,
    format="%(asctime)s %(levelname)s %(name)s - %(message)s",
)
logger = logging.getLogger("safeassist")
...
async def ask(payload: AskRequest, request: Request) -> StreamingResponse:
    ...
    Pipeline:
      1. Injection classifier over the RAW query (see the note below).
      2. Input scrubber over a SEPARATE COPY, for egress.
    ...
    # ============================================================ TASK 1
    # The ordering below is GuardianAI's, and it is wrong. Reading 3 has the
    # trace; the short version is that the scrubber runs first, replaces the
    # token the classifier's pattern depends on, and the classifier then sees
    # text the attack is no longer in.
    #
    # Reproduce it before you fix it:
    #
    #     pytest -q tests/test_safeassist.py -k task1
    #
    # Fix it so that:
    #   * the classifier is given the RAW query
    #   * the scrubber still runs, on its own copy, for egress
    #   * BOTH audit events survive: an attack carrying personal data must
    #     produce an injection_refusal AND a pii_scrub, not one or the other
    #   * the log-only band (a detection the scrubber recorded but did not
    #     redact) creates a review record. That is TASK 4; wire it here.
    #
    ...
    # ============================================================ TASK 3
    # The fifth egress path is not instrumented. Nothing here writes
    # operational telemetry, so `app/telemetry.py` is never called and the
    # tests for it fail.
    #
    # Emit one telemetry line per answered request. Then read
    # tests/test_safeassist.py -k task3: the file on disk must never contain a
    # raw value, and the FIELDS matter as much as the message.
    # ============================================================
...
#
# These four routes are the backend Week 13 renders approval buttons over. They
# are deliberately separate from /ask: a reviewer is a different person, with a
# different role, acting minutes or hours later.


@app.get("/review/pending")
@rbac.requires_role("clinician")
async def review_pending(request: Request) -> dict:
    """Everything waiting on a human. Never carries a detected value."""
    return {"pending": [r.model_dump() for r in review.pending()]}


@app.get("/review/stats")
@rbac.requires_role("clinician")
async def review_stats(request: Request) -> dict:
    """Counts by state. `pending` is the number a dashboard would alert on."""
    return review.stats()


@app.post("/review/{review_id}/decide")
@rbac.requires_role("clinician")
async def review_decide(review_id: str, payload: ReviewDecision, request: Request) -> dict:
    """Approve or reject one pending record.

    A decided record is terminal. Deciding it again is a 409 rather than a
    silent overwrite, because an audit trail in which a decision can be
    replaced is not an audit trail.
    """
    rid = request.state.request_id
    try:
        rec = review.decide(
    ...
    audit.write_event(
        ...
        action_taken=rec.state,
    )
    return rec.model_dump()


@app.post("/review/sweep")
@rbac.requires_role("clinician")
async def review_sweep(request: Request) -> dict:
    """Escalate anything past the TTL. Intended to run on a schedule.

    A queue with no timeout is a backlog that nobody has noticed yet.
    """
    escalated = review.sweep()
    return {"escalated": len(escalated), "review_ids": [r.review_id for r in escalated]}
