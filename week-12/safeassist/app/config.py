...
class Settings(BaseSettings):
    ...
    qdrant_api_key: str = ""
    qdrant_collection: str = "safeassist"
    ...
    # ---- SafeAssist additions ----
    # The human-review queue. The 0.35 to 0.65 band no longer just writes a log
    # line; it creates a review record a person can act on. `review_ttl_hours`
    # is how long a record may sit in `pending` before it is escalated, because
    # a queue with no timeout is a queue that silently becomes a backlog.
    review_queue_path: str = "./review_queue.jsonl"
    review_ttl_hours: int = 24
    review_default_reviewer: str = "unassigned"

    # Telemetry. A second egress path that is NOT the audit log: the ordinary
    # operational logging a service does. It leaves your trust boundary like
    # anything else, usually with longer retention and wider access.
    telemetry_path: str = "./telemetry.jsonl"
    telemetry_enabled: bool = True

    # Domain entity. Medical record numbers are the entity this deployment
    # actually holds and that no general-purpose detector has heard of.
    mrn_score: float = 0.85
