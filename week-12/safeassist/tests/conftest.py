...
os.environ["REVIEW_QUEUE_PATH"] = os.path.join(_TMP, "review_queue.jsonl")
os.environ["TELEMETRY_PATH"] = os.path.join(_TMP, "telemetry.jsonl")
...
REVIEW_LOG = os.environ["REVIEW_QUEUE_PATH"]
TELEMETRY_LOG = os.environ["TELEMETRY_PATH"]
...
def _stub_stream_answer(*, system_prompt: str, user_message: str):
    ...
    # Leading-space tokens, which is what a real tokeniser emits. GuardianAI's
    # stub yielded `word + " "`, the tidy shape, and that convenience is exactly
    # what hid its sentence-buffer defect from its own test suite. Stub the
    # awkward shape the real thing produces, not the one you would have designed.
