...
Copied from CitationRAG (app/embedder.py) - same embedding model,
same vector dimension (3072), same SHA-256 cache so re-querying the same
text never costs an extra API call.

No week-7-specific logic here. Swap embed_model in config to use a
different model; the rest of the pipeline is unchanged.
"""
